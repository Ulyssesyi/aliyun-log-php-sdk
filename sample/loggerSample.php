...
use Aliyun\Log\LoggerFactory;
use Aliyun\Log\Models\ListShipperResponse;
use Aliyun\Log\Models\LogItem;
use Aliyun\Log\Models\OssShipperConfig;
use Aliyun\Log\Models\OssShipperCsvStorage;
use Aliyun\Log\Models\OssShipperJsonStorage;
use Aliyun\Log\Models\OssShipperParquetStorage;
...
$logger = LoggerFactory::getLogger($client, $project, $logstore);
$logMap = [
    'message' => 'tet',
    'haha' => 'hehe',
];
...
$anotherLogger = LoggerFactory::getLogger($client, $project, $logstore);

//create different logger instance by calling factory method with topic parameter defined
$batchLogger = LoggerFactory::getLogger($client, $project, $logstore, 'helloworld');
...
function updateShipper($client, $project, $logstore, $shipperName) {
    ...
    $updateShipper = new UpdateShipperRequest($project);
...
function getAndRetryShipperTasks(Client $client, $project, $logstore, $shipperName): void {
    $getShipperTasks = new GetShipperTasksRequest($project);
    ...
    $retryShipperTask = new RetryShipperTasksRequest($project);
...
function deleteShipper(Client $client, $project, $logstore, $shipperName): void {
    //try delete the existing shipper
    $deleteShipper = new DeleteShipperRequest($project);
...
function getShipperCommonConfig(OssShipperStorage $ossShipperStorage) {
    $ossConfig = new OssShipperConfig();
...
function createCommonShipper($project, $logstore, $shipperName) {
    ...
    $shipper = new CreateShipperRequest($project);
...
function createCsvShipper(Client $client, $project, $logstore): void {
    $shipper = createCommonShipper($project, $logstore, 'testcsvshipper');

    $ossCsvStorage = new OssShipperCsvStorage();
    $ossCsvStorage->setColumns(['__topic__',
        'alarm_count',
        'alarm_message',
        'alarm_type',
        'category',
 ...
 * @param $logstore
 * @param $shipperName
 * @return GetShipperConfigRequest
 */
function getShipperConfig(Client $client, $project, $logstore, $shipperName) {

    $getShipperConfig = new GetShipperConfigRequest($project);
 ...
 */
function listShipper(Client $client, $project, $logstore) {
    $listShipper = new ListShipperRequest($project);
 ...
 * @param $logstore
 */
function createParquetShipper(Client $client, $project, $logstore): void {
    //create shipper with json storage
    $shipper = createCommonShipper($project, $logstore, 'testparquetshipper');

    $ossParquetStorage = new OssShipperParquetStorage();
    $ossParquetStorage->setColumns([
        [
            'name' => '__topic__',
            'type' => 'string',
        ],
 ...
 * @param $logstore
 */
function createJsonShipper(Client $client, $project, $logstore): void {
    // create a json shipper
    $ossJsonStorage = new OssShipperJsonStorage();
